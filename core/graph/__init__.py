"""
=============================================================================
执行子图（Execution Subgraph）—— 感知 + 规划 + ReAct 执行循环
=============================================================================
...
【可重入设计】
    这个子图可以被主图反复调用：
    - 第一次进入：analyzer 先做需求分析 → 拆解子任务 → ReAct worker 逐个执行
    - 再次进入（被主图触发重试）：跳过 analyzer，ReAct worker 带着报错修复
...
# 如需切回旧版 worker，取消下面注释并替换 react_worker
...
def route_entry(state: AgentState) -> str:
    ...
    直接跳转到 worker，跳过 analyzer 的重复分析。
...
def build_execution_subgraph():
    ...
    if USE_REACT_WORKER:
        react_subgraph = build_react_worker_subgraph()
