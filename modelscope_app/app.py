...
【部署方式（魔塔社区）】
  ...
  2. 在魔塔 Space 设置中配置环境变量（Settings → Environment Variables）：
     ...
     - AUTH_USER        : 登录用户名
     - AUTH_PASS        : 登录密码
  3. Space SDK 选 Gradio，入口文件指向 app.py

【本地运行】
  cd Agent_upgrade && python app.py
...
【新增——密码验证】
  新增全局登录验证，账号密码从环境变量读取，本地运行可在 .env 文件配置
=============================================================================
"""
...
# 【新增】读取登录账号密码（优先环境变量，无则设默认值）
AUTH_USER = os.getenv("AUTH_USER", "admin")
AUTH_PASS = os.getenv("AUTH_PASS", "123456")
...
from modelscope_app.ui_helpers import (  # noqa: E402
    list_workspace,
    get_session_list,
    preview_file,
    clear_workspace,
...
def list_sessions(show_completed: bool = False):
    """刷新断点续传下拉列表 → 返回 gr.Dropdown.update(choices=...)"""
    items = get_session_list(checkpointer, app_graph, show_completed=show_completed)
    return gr.update(choices=items, value=None)


def on_embed_mode_change(mode: str):
    """向量模型模式切换 → 控制输入框显隐"""
    is_local = (mode == "local")
    return (
        gr.update(visible=is_local),      # embed_path
        gr.update(visible=not is_local),  # embed_url
        gr.update(visible=not is_local),  # embed_key
        gr.update(visible=not is_local),  # embed_model_name
    )


def on_embed_load(mode: str, path: str, url: str, key: str, name: str):
    """加载向量模型"""
    return load_embedding_model(
        mode=mode,
        model_path=(path or "").strip(),
...
demo = build_ui(
    ...
    on_embed_mode_change=on_embed_mode_change,
    on_embed_load=on_embed_load,
    auth_user=AUTH_USER,
    auth_pass=AUTH_PASS,
)
